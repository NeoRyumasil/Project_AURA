...
class FasterQwen3TTS:
    ...
    def _prepare_generation(
        ...
        """
        if instruct:
            # For 1.7B Instruct/CustomVoice models, injecting instruct tags enables emotion
            input_texts = [self.model._build_instruct_text(instruct) + self.model._build_assistant_text(text)]
        else:
